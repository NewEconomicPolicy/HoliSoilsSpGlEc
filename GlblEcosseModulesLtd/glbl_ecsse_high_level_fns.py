...
def _generate_ecosse_files(form, climgen, mask_defn, yield_df, num_band, yield_defn, pi_var, pi_csv_tple):
    """
    ...
    print('Getting historic data for band {}'.format(num_band) )
    #      ==============================
    if wthr_rsrc == 'HARMONIE':
        pettmp_hist = climgen.fetch_harmonie_NC_data(aoi_indices_fut, num_band)
    ...
    # ===============================================================================================
    yrs_pi = None
    ltd_data = make_ltd_data_files.MakeLtdDataFiles(form, climgen, yrs_pi, comments=True)   # create limited data object
    ...
    for site_indx, site_rec in enumerate(aoi_res):
        ...
        last_time = update_progress(last_time, start_time, completed, num_meta_cells, skipped, warning_count)
...
def generate_banded_sims(form):
    ...
    elif form.w_use_pi_csv.isChecked():
        pi_csv_fname = form.w_lbl_pi_csv.text()
        pi_csv_tple = cnvrt_joe_plant_inputs_to_df(pi_csv_fname)
